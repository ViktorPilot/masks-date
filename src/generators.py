...
def filter_by_currency(transactions_: list[dict], type_currency: str) -> Generator:
    """Функция, возвращающая итератор с заданным значением валюты"""
    return (
        transaction
        for transaction in transactions_
        if transaction.get("operationAmount", {}).get("currency", {}).get("code", {}) == type_currency
    )


def transaction_descriptions(transactions_: list[dict]) -> Generator:
    """Функция, возвращающая описание проведенных операций"""
    gen_transactions = (
        transaction.get("description", "") for transaction in transactions_ if "description" in transaction
    )
