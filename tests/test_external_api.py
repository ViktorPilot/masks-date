...
def test_get_amount_transactions_not_amount(not_amount: dict) -> None:
    ...
    assert get_amount_transactions(not_amount) == "Отсутствует сумма транзакции"
...
def test_get_amount_transactions_invalid_type_amount(invalid_type_amount: dict) -> None:
    ...
    assert get_amount_transactions(invalid_type_amount) == "Неправильный тип данных суммы транзакции"
...
def test_get_amount_transactions_invalid_code_currency(invalid_code_currency: dict) -> None:
    ...
    assert get_amount_transactions(invalid_code_currency) == "В транзакции не указан или указан неверно тип валюты"


def test_get_amount_transactions_not_list_and_empty_dict() -> None:
    """Тестирование функции с пустым словарем"""
    assert get_amount_transactions({}) == "Отсутствуют корректные данные по транзакции"
...
def test_get_amount_transactions_stat_code_not_200(mock_requests, get_amount_transactions_usd) -> None:
    ...
    assert get_amount_transactions(get_amount_transactions_usd) == "При конвертации валюты произошла ошибка 300"
